...
static NSString *serHost = @"";
static NSString *awUISDKVersion = @"2.0.0.1";


@interface AWHttpManager ()
...
@implementation AWHttpManager

+ (instancetype)sharedInstance {
    static AWHttpManager * sharedInstance;
...
- (void)requestWithPath:(NSString *)path extraParams:(NSDictionary * _Nullable)params completion:(nullable void (^)(NSInteger result, NSString * errorMsg, NSDictionary * _Nullable data))completion {
    ...
    dispatch_async(self.networkQueue, ^{
        [weakSelf.sessionManager POST:[NSString stringWithFormat:@"%@%@", serHost, path] parameters:[weakSelf processedParamsWithExtraParams:params] headers:[weakSelf basicHeader] progress:^(NSProgress * _Nonnull uploadProgress) {
